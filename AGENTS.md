...
- 수집/관리 모달의 목록 재조회 버튼은 모달 열기, 추가/삭제 후 자동 갱신, 상태 폴링으로 대체 가능하면 노출하지 않는다. 꼭 필요한 수동 재조회 액션은 `새로고침`으로 명칭을 통일한다.
- 모달, 드로어, 알림 패널처럼 배경 스크롤을 막아야 하는 UI는 `features/platform/components/use-body-scroll-lock.ts`의 `useBodyScrollLock`을 사용한다. 각 컴포넌트에서 `document.body.style.overflow`, `position`, `top`, `touchAction`을 직접 조작하지 않는다. 중복 잠금은 페이지 스크롤이 복구되지 않는 결함을 만들 수 있다.
