...
export async function POST(request: Request) {
  try {
    ...
    if (!serviceName) {
      return NextResponse.json({ message: '시술명을 입력해주세요.' }, { status: 400 })
    }

    const sourcePrompt = createNaverBlogPrompt({
      serviceName,
      keyBenefits: normalizeBenefits(body.keyBenefits),
    })
    ...
    return NextResponse.json({
      ...
      topic: serviceName,
      modelName: process.env.GEMINI_TEXT_MODEL ?? 'gemini-2.5-flash',
      generatedContent,
      sourcePrompt,
    })
  } catch (error) {
