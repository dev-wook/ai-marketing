...
import { NaverApiError, searchNaverBlog } from '@/lib/naver'

export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url)
    const query = searchParams.get('query')?.trim() ?? ''
    const display = toInteger(searchParams.get('display'), 20)
...
function toNaverSearchErrorResponse(error: unknown) {
  ...
  if (error instanceof Error) {
    console.error('Naver blog search error', {
  ...
  return {
    status: 500,
    message: '네이버 블로그 검색 정보를 불러오는 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요.',
  }
}
