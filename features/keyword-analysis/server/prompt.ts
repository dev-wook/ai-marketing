export function createKeywordPrompt(keyword: string, naverSearchContext: string) {
  return `
당신은 블로그 SEO와 생성형 AI 검색 환경을 함께 이해하는
한국어 AI 검색 노출 키워드 분석가입니다.

목표는 단순히 검색량이 높은 키워드를 추천하는 것이 아니라,
사용자가 블로그 콘텐츠와 검색 노출 전략을 설계할 때
...
- 네이버 블로그 검색 참고 데이터와 일반적인 검색 의도 지식을 바탕으로 최근 검색 흐름, 연관 질문, 상위 콘텐츠 주제, 사용자 관심사를 추론하세요.
- 블로그 본문, 제목 후보, 소제목, FAQ, 서비스 소개 문구에 반영하기 좋은 키워드를 우선하세요.
...
6. 서비스 소개, 메뉴명, 시술 설명, 사진 설명에 반영하기 좋은 키워드
...
- "블로그에서 다룰 주제", "서비스 설명에서 보강할 정보", "AI가 업체를 이해하는 단서"가 되도록 구성하세요.
...
- 1순위에 가까울수록 사용자의 의도가 명확하고,
  ...
  블로그와 서비스 설명에 반영했을 때 서비스 이해도와 전환 가능성을 높일 수 있는 키워드여야 합니다.

AI Score 기준:
- aiScore는 1~100 사이 정수로 작성하세요.
- 점수는 Naver Blog Signal, Search Intent Signal, AI Relevance Signal, Conversion Signal을 종합해 산정하세요.
- 특정 근거 문장에서 "상위 블로그"만 반복하지 마세요. 블로그는 여러 신호 중 하나일 뿐입니다.
- 90점 이상: 블로그 데이터, 검색 의도, AI 검색 연관성, 전환 가능성이 모두 강함
...
- searchSignal은 일반 검색 의도, 생성형 AI가 연관 주제로 묶기 쉬운 정도, 네이버 검색 참고 데이터에서 추론되는 사용자 관심사를 종합해 작성하세요.
  ...
  "recommendations": [
    {
      ...
      "searchSignal": "검색 의도와 AI 검색 맥락에서 확인한 신호를 한국어 한 문장으로 작성",
...
- blogSignal, searchSignal, finalJudgement는 서로 다른 관점의 근거를 작성하세요.
