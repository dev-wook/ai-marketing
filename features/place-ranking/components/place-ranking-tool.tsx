      ...
      {weekly.dailyForecasts.length ? (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {weekly.dailyForecasts.map((item) => (
            <span
              key={`${item.label}-${item.expectedBookings}`}
              className="rounded-full bg-cyan-300/10 px-2 py-1 text-[11px] font-black text-cyan-100"
            >
              {item.label} {item.expectedBookings}
            </span>
          ))}
        </div>
      ) : null}
